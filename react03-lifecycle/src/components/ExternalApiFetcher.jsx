...
function RandomUser(props){
       /*
        외부 서버의 API를 이용해서 얻어온 데이터를 저장할 상태변수를 
        생성해준다.
        초깃값은 JSON 포맷에 따라 달라질 수 있으므로 확인 후 설정해준다.
        */
    const [myJSON, setMyJSON] = useState({results:[]});

    //컴포넌트 렌더링이 완료된 후 API정보를 얻어 온다.
    useEffect(function(){
     //API 서버에서 10명의 정보를 JSON 으로 받아온다.
        ...
        .then((json)=>{
            console.log(json);
            //상태변수를 변경해서 화면을 리렌더링 한다.
            setMyJSON(json);
        });
    }, []);
    /*
    의존성 배열은 빈 배열로 설정하여, 최초 한번만 실행되도록 한다.
    이 설정이 없으면 무한루프에 빠지게 된다.
    */


    /*
    API를 통해ㅔ서 얻어온 User의 정보로 반복해서 tr 태그를 구성한다.
    */
    let trTag = myJSON.results.map((data)=>{
        return (
            //tr 태그에는 중복되지 않는 key prop 를 설정하여 준다.
            <tr key={data.login.md5}>
                {/* 프로필이미지, 이름, 아이디 등의 정보 파싱 후 출력 */}
                <td><img src={data.picture.thumbnail} alt={data.login.username} /></td>
                <td><a href="/" onClick={(e)=>{
                    e.preventDefault();
                    /*
                    아이디를 클릭하면 프롭스로 전달받은 함수를 통해 
...
function ExternalApiFetcher(){
    return (<>
        <h2>외부 서버 통신</h2>
        {/* 링크를 클릭하면 User의 정보를 alert 창으로 
        띄워주기 위한 함수를 프롭스로 전달해준다. */}
        <RandomUser onProfile={(sData)=>{
            console.log(sData);
            // 템플릿 리터럴을 통해 + 기호 없이 문자열을 하나로 연결해서 출력해준다.
