...
const GlobalTop = (props) => {
    ...
    /*
    목록의 데이터로 사용할 스테이트 생성.
    초깃값은 빈 배열로 설정.
    차후 useEffect가 실행되면 Json파일의 내용을 
    읽어와서 값을 변경한다.
    */
    const[myList, setMyList] = useState([]);

    //이 컴포넌트가 1차 렌더링 된 후 실행되는 훅
    useEffect(() => {
        console.log('3. useEffect 실행');
        // 프로젝트 내부에 있는 json 파일을 get방식으로 요청.
        fetch('./json/myData.json')
        .then((result) =>{
            /*
            요청에 성공하면 json파일의 데이터가 매개변수 result를 통해 반환된다.
            반환된 데이터는 Text형식이므로
            JSON 포맷으로 변환한 후 반환된다.
            */
            return result.json();
        })
        /*
    ...
    만약 의존성 배열을 생략하면 무한히 반복 로딩되는 현상이 발생된다.
    useEffect의 내부에서 상태(State,스테이트)를 변경하는 코드가 있기 때문이다.
    */


    /*
    최초  실행에서는 myList가 빈 배열이므로 아무런 내용도 출력되지 않는다.
    따라서 틀만 만들어진 상태로 렌더링 된다.
    */
    let listTag = myList.map((data)=>{
        return(
            <li key={data.id}> 
            {/* data-id 속성에 설정된 값은 합성 이벤트 객체의 target
            속성 하위의 dataset.id를 통해 읽어올 수 있따.
            이 속성에 게시물의 일련번호인 num을 설정하고 있다. */}
                <a href={data.id} data-id={data.num} 
                onClick={(e)=>{
                    console.log('합성 이벤트 객체',e);
                    e.preventDefault();
...
function LocalJsonFetcher() {
    ...
    return (<>
        <h2>내부 서버 통신</h2>
        {/* 클릭시 프로젝트 내부에 저장된 dto.json 파일을
        get방식으로 요청한 후 콜백 데이터를 받아오는 기능의 함수를
        정의한 후 프롭스로 전달한다. 
        자식 컴포넌트는 이 함수를 호출할때 게시물의 일련번호를 
        인수로 전달한다. */}
        <GlobalTop myLinkClick={(num)=>{
            ...
            .then((json)=>{
                //JSON 파일의 내용으로 스테이트를 변경하고 리렌더링한다.
                console.log(json);
                setMyResult(json);
            });
        }}></GlobalTop>
